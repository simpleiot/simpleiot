	...
	return _Utils_Tuple2(
		{
			...
			deviceEdits: {device: elm$core$Maybe$Nothing, visibility: rundis$elm_bootstrap$Bootstrap$Modal$hidden},
			devices: {devices: _List_Nil, dirty: false},
	...
	return elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				A2(rundis$elm_bootstrap$Bootstrap$Accordion$subscriptions, model.accordionState, author$project$Main$AccordionMsg),
				A2(elm$time$Time$every, 1000, author$project$Main$Tick)
			]));
};
var author$project$Main$UpdateDevices = function (a) {
	return {$: 'UpdateDevices', a: a};
};
var author$project$Main$Device = F3(
	function (id, config, state) {
		return {config: config, id: id, state: state};
	});
var author$project$Main$DeviceConfig = function (description) {
	return {description: description};
};
var elm$json$Json$Decode$string = _Json_decodeString;
var author$project$Main$deviceConfigDecoder = A2(
	elm$json$Json$Decode$map,
	author$project$Main$DeviceConfig,
	A2(elm$json$Json$Decode$field, 'description', elm$json$Json$Decode$string));
var author$project$Main$DeviceState = function (ios) {
	return {ios: ios};
};
var author$project$Main$Sample = F3(
	function (id, value, time) {
		return {id: id, time: time, value: value};
	});
var elm$json$Json$Decode$float = _Json_decodeFloat;
var elm$json$Json$Decode$map3 = _Json_map3;
var author$project$Main$sampleDecoder = A4(
...
var author$project$Main$samplesDecoder = elm$json$Json$Decode$list(author$project$Main$sampleDecoder);
var author$project$Main$deviceStateDecoder = A2(
	elm$json$Json$Decode$map,
	author$project$Main$DeviceState,
	A2(elm$json$Json$Decode$field, 'ios', author$project$Main$samplesDecoder));
var author$project$Main$deviceDecoder = A4(
	elm$json$Json$Decode$map3,
	author$project$Main$Device,
	A2(elm$json$Json$Decode$field, 'id', elm$json$Json$Decode$string),
	A2(elm$json$Json$Decode$field, 'config', author$project$Main$deviceConfigDecoder),
	A2(elm$json$Json$Decode$field, 'state', author$project$Main$deviceStateDecoder));
var author$project$Main$devicesDecoder = elm$json$Json$Decode$list(author$project$Main$deviceDecoder);
...
var author$project$Main$urlDevices = A2(
...
var author$project$Main$apiGetDevices = A2(
	elm$http$Http$send,
	author$project$Main$UpdateDevices,
	A2(elm$http$Http$get, author$project$Main$urlDevices, author$project$Main$devicesDecoder));
var author$project$Main$DeviceConfigPosted = function (a) {
	return {$: 'DeviceConfigPosted', a: a};
};
...
var author$project$Main$deviceConfigEncoder = function (deviceConfig) {
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'description',
				elm$json$Json$Encode$string(deviceConfig.description))
			]));
};
...
var author$project$Main$apiPostDeviceConfig = F2(
	function (id, config) {
		var url = A2(
			elm$url$Url$Builder$absolute,
			_List_fromArray(
		...
		var body = elm$http$Http$jsonBody(
			author$project$Main$deviceConfigEncoder(config));
		return A2(
			elm$http$Http$send,
			author$project$Main$DeviceConfigPosted,
			A3(elm$http$Http$post, url, body, author$project$Main$responseDecoder));
	});
...
var author$project$Main$findDevice = F2(
	function (devices, id) {
		return A2(
...
var author$project$Main$updateDevice = F2(
	function (devices, device) {
		if (device.$ === 'Nothing') {
			return devices;
		} else {
			var deviceUpdate = device.a;
			var index = A2(
				elm_community$list_extra$List$Extra$findIndex,
				function (d) {
					return _Utils_eq(d.id, deviceUpdate.id);
				},
				devices);
			var devicesModified = function () {
				if (index.$ === 'Nothing') {
					return A2(
					...
					var i = index.a;
					return A3(elm_community$list_extra$List$Extra$setAt, i, deviceUpdate, devices);
				}
			}();
			return devicesModified;
		}
	});
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'Tick':
				var newTime = msg.a;
				return _Utils_Tuple2(model, author$project$Main$apiGetDevices);
			case 'UpdateDevices':
				var result = msg.a;
				var _n1 = model.devices.dirty;
				if (_n1) {
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
				} else {
					if (result.$ === 'Ok') {
						var devicesUpdate = result.a;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									devices: {devices: devicesUpdate, dirty: false}
								}),
							elm$core$Platform$Cmd$none);
					} else {
						var err = result.a;
						var _n3 = A2(elm$core$Debug$log, 'UpdateDevices error: ', err);
						return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
					}
				}
			case 'DeviceConfigPosted':
				var result = msg.a;
				var devices = model.devices;
				var newDevices = _Utils_update(
					devices,
					{dirty: false});
				var newModel = _Utils_update(
					model,
					{devices: newDevices});
				var _n4 = A2(elm$core$Debug$log, 'DeviceConfigPosted', result);
				if (_n4.$ === 'Ok') {
					var string = _n4.a;
					return _Utils_Tuple2(newModel, elm$core$Platform$Cmd$none);
				} else {
					var err = _n4.a;
					var _n5 = A2(elm$core$Debug$log, 'DeviceConfigPosted error: ', err);
					return _Utils_Tuple2(newModel, elm$core$Platform$Cmd$none);
				}
			case 'EditDevice':
				var id = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							deviceEdits: {
								device: A2(author$project$Main$findDevice, model.devices.devices, id),
								visibility: rundis$elm_bootstrap$Bootstrap$Modal$shown
							}
						}),
					elm$core$Platform$Cmd$none);
			case 'EditDeviceClose':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
			...
			case 'EditDeviceSave':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							deviceEdits: {device: model.deviceEdits.device, visibility: rundis$elm_bootstrap$Bootstrap$Modal$hidden},
							devices: {
								devices: A2(author$project$Main$updateDevice, model.devices.devices, model.deviceEdits.device),
								dirty: true
							}
						}),
					function () {
						var _n6 = model.deviceEdits.device;
						if (_n6.$ === 'Nothing') {
							return elm$core$Platform$Cmd$none;
						} else {
							var dev = _n6.a;
							return A2(author$project$Main$apiPostDeviceConfig, dev.id, dev.config);
						}
					}());
			default:
				var desc = msg.a;
				var _n7 = model.deviceEdits.device;
				if (_n7.$ === 'Nothing') {
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
				} else {
					var device = _n7.a;
					var deviceEdits = model.deviceEdits;
					var deviceConfig = device.config;
					var newDeviceConfig = _Utils_update(
						deviceConfig,
						{description: desc});
					var newDevice = _Utils_update(
						device,
						{config: newDeviceConfig});
					var newDeviceEdits = _Utils_update(
						deviceEdits,
						{
							device: elm$core$Maybe$Just(newDevice)
						});
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{deviceEdits: newDeviceEdits}),
						elm$core$Platform$Cmd$none);
				}
		}
	});
var author$project$Main$EditDevice = function (a) {
	return {$: 'EditDevice', a: a};
};
var author$project$Main$renderDeviceSummary = function (dev) {
	return dev.config.description + (' (' + (dev.id + ')'));
};
...
var author$project$Main$renderIos = function (samples) {
	return rundis$elm_bootstrap$Bootstrap$Accordion$listGroup(
		A2(
			elm$core$List$map,
			function (s) {
				return A2(
					...
					_List_fromArray(
						[
							elm$html$Html$text(
							s.id + (': ' + A2(myrho$elm_round$Round$round, 2, s.value)))
						]));
			},
			samples));
};
...
var danmarcab$material_icons$Material$Icons$Image$edit = A2(
...
var rundis$elm_bootstrap$Bootstrap$Accordion$appendHeader = F2(
	function (elements, _n0) {
		...
		return rundis$elm_bootstrap$Bootstrap$Accordion$Header(
			_Utils_update(
				header_,
				{
					childrenPostToggle: _Utils_ap(header_.childrenPostToggle, elements)
				}));
	});
...
var author$project$Main$renderDevice = function (dev) {
	return rundis$elm_bootstrap$Bootstrap$Accordion$card(
		{
			blocks: _List_fromArray(
				[
					author$project$Main$renderIos(dev.state.ios)
				]),
			header: A2(
				rundis$elm_bootstrap$Bootstrap$Accordion$appendHeader,
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								...
								author$project$Main$EditDevice(dev.id)),
								elm$html$Html$Attributes$class('btn btn-light')
							]),
						_List_fromArray(
							[
								A2(danmarcab$material_icons$Material$Icons$Image$edit, avh4$elm_color$Color$black, 25)
							]))
					]),
				A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										elm$html$Html$text(
										author$project$Main$renderDeviceSummary(dev))
									]))
							])))),
			id: dev.id,
			options: _List_Nil
...
var author$project$Main$renderDevices = function (model) {
	return A2(
		rundis$elm_bootstrap$Bootstrap$Accordion$view,
		model.accordionState,
		A2(
			rundis$elm_bootstrap$Bootstrap$Accordion$cards,
			A2(elm$core$List$map, author$project$Main$renderDevice, model.devices.devices),
			rundis$elm_bootstrap$Bootstrap$Accordion$withAnimation(
				rundis$elm_bootstrap$Bootstrap$Accordion$config(author$project$Main$AccordionMsg))));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				author$project$Main$renderDevices(model)
			]));
};
			...
			A3(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$src('/public/farmation-logo.png'),
								elm$html$Html$Attributes$width(83),
...
var author$project$Main$EditDeviceChangeDescription = function (a) {
	return {$: 'EditDeviceChangeDescription', a: a};
};
var author$project$Main$EditDeviceClose = {$: 'EditDeviceClose'};
var author$project$Main$EditDeviceSave = {$: 'EditDeviceSave'};
...
var author$project$Main$renderEditDevice = function (deviceEdits) {
	var _n0 = deviceEdits.device;
	if (_n0.$ === 'Nothing') {
		return A2(
			rundis$elm_bootstrap$Bootstrap$Modal$view,
			deviceEdits.visibility,
			A3(
				rundis$elm_bootstrap$Bootstrap$Modal$footer,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										elm$html$Html$Events$onClick(author$project$Main$EditDeviceClose)
									]))
							]),
						_List_fromArray(
							[
				...
				A3(
					...
					_List_fromArray(
						[
							elm$html$Html$text('No device to edit')
						]),
					A3(
						...
						rundis$elm_bootstrap$Bootstrap$Modal$small(
							rundis$elm_bootstrap$Bootstrap$Modal$config(author$project$Main$EditDeviceClose))))));
	} else {
		var device = _n0.a;
		return A2(
			rundis$elm_bootstrap$Bootstrap$Modal$view,
			...
			A3(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										elm$html$Html$Events$onClick(author$project$Main$EditDeviceSave)
									]))
							]),
						_List_fromArray(
							[
				...
				A3(
						...
						[
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											elm$html$Html$text('Device description')
										])),
										...
										[
											...
											_List_fromArray(
												[
													...
													elm$html$Html$Events$onInput(author$project$Main$EditDeviceChangeDescription),
													elm$html$Html$Attributes$value(device.config.description)
												]))
										]))
								]))
						]),
					A3(
						...
						_List_fromArray(
							[
								elm$html$Html$text('Edit device (' + (device.id + ')'))
							]),
						rundis$elm_bootstrap$Bootstrap$Modal$small(
							rundis$elm_bootstrap$Bootstrap$Modal$config(author$project$Main$EditDeviceClose))))));
	}
			...
			[
				...
				_List_fromArray(
					[
						...
						author$project$Main$renderEditDevice(model.deviceEdits)
					]))
			]),
		title: 'Farmation'
	};
