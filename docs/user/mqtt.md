...
Everything on this page is planned rather than implemented; it is documented
now so you can plan deployments around it and so the design is open for
discussion on the
[community forum](https://community.tmpdir.org/c/simple-iot/5). See the
[PLC page](plc.md) for how MQTT compares with the other ways to bring plant
data in, and [ADR-8](../adr/8-iot-data-models.md) for how common MQTT payload
formats compare with the Simple IoT point model.
...
2. **Subscriptions.** An `mqtt` node and its `mqttSub` children map named
   topics into points.
3. **A topic schema.** Declare what your topic levels mean, and nodes are
   created automatically as data arrives.
4. **Sparkplug B.** Birth certificates describe the data, so the node
   structure builds itself. Covered in its own section below.
...
- The broker implements **MQTT 3.1.1**. Clients that require MQTT 5 are
  refused, which matters mostly for newer gateways that default to version 5.
...
- When an auth token is configured (`SIOT_AUTH_TOKEN`), MQTT clients supply it
  ...
  (`SIOT_NATS_TLS_CERT`/`SIOT_NATS_TLS_KEY`, which also serve the MQTT
...
- Published messages become NATS subjects, so anything connected to Simple IoT
  over NATS sees them. Topic levels convert as `/` to `.`, and a literal `.`
  in a topic converts to `//`. A Sparkplug topic of
...
JSON payloads are the first target, since they cover the AWS IoT, Azure IoT,
and gateway-defined formats most installations use. How a payload maps depends
on `path`:
...
- **`path` blank, payload is a bare number or string:** the payload itself
  becomes the point.
- **`path` blank, payload is an object:** each top-level field becomes a
  point, with the field name as the point key. A payload with twenty fields
  becomes one node and twenty points rather than twenty nodes, and the field
  name is queryable in the database as the `key` label.
...
individually when they represent different things. The topic schema below is
the better tool when you want one rule to cover many topics.

## Automatic nodes with a topic schema (planned)

Plain MQTT carries no information about which topic level is a site and which
is a device, which is why nothing is created automatically by default. A topic
...
- **Each named level becomes a node**, carrying a tag named by its schema
  label. Intermediate levels are group nodes; the last named level is an
  `mqttDevice` node that receives the points.
- **Everything beyond the named levels becomes the point key.** Remaining
  topic levels and JSON field names join into the key, so a deeper topic
  extends the key rather than the node tree.
- **Nodes are matched by topic identity, not by name.** Renaming a
  description or adding tags to an auto-created node survives later messages
  and restarts, and nothing is duplicated.
- **Nodes are never deleted automatically.** A quiet sensor and a removed
  sensor look the same from outside, so removal stays a human decision.
...
[Sparkplug B](https://sparkplug.eclipse.org/) adds a defined topic namespace,
a protobuf payload, and birth and death certificates on top of MQTT. Because
an edge node announces every metric it will report, with names and types,
Simple IoT builds the node structure from the data itself and no schema or
subscription list is required. Enable it on the `mqtt` node:
...
- **NBIRTH and DBIRTH** create or refresh the group, edge node, and device
  nodes and write one point per metric. A birth after a gateway restart
  refreshes the existing nodes rather than duplicating them, and tags or
  descriptions you have set on them survive.
- **NDATA and DDATA** arrive as point updates carrying the payload timestamp.
  ...
  mapping for an alias, such as after its own restart, it requests a rebirth
  and the structure rebuilds.
...
hierarchies, tags, and point keys become queryable series, and the cautions
that come with them: keep unbounded values out of tags, and settle names
before collecting history you intend to keep.
...
- **Per-client credentials**, so each gateway authenticates individually and
  can be restricted to its own topics.
