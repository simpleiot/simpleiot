...
`..` is refused. Matching by name is preferred, since the device number depends
on probe order and is not stable across boots. The client publishes the resolved
`deviceName` and `devicePath` back to the node either way, so it is always clear
