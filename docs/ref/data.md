...
- eliminate echos where a client may be subscribed to a subject as well as
  ...
  if it was the author of a point it receives, and if so simply drop it. Clients
  can also create a new NATS connection with the NoEcho option to handle this
  situation.
