...
func main() {
	...
	if RegSel == nil {
		log.Fatal("GPIO register bad number for RegSel")
	}
	LcdReset := gpioreg.ByName(LcdReset_Gpio)
	if LcdReset == nil {
		log.Fatal("GPIO register bad number for LcdReset")
	}

	PwmGpio := gpioreg.ByName(LcdPwm_Gpio)
	if PwmGpio == nil {
		log.Fatal("GPIO register bad number for PwmGpio")
	}

	err := PwmGpio.Out(gpio.High)
	if err != nil {
